...
Numerical audit &
Curved radial-ring quadrature on the actual surface in $\mathbb R^4$, using finite-difference tangents and no analytic pullback density, agrees with $A(E)$ to $1.1\times10^{-11}$ relative and recovers $T=dA/dE$ to $8.3\times10^{-11}$. Independent constrained-flow quadrature also agrees to $8.3\times10^{-11}$.\\
\end{tabularx}
...
Attachments: exact verifier, constrained-flow audit, curved-area audit, flat-mesh audit, crossing data, figure code, JSON payload, and this LaTeX source.
...
\embedfile[desc={Trefoil independent Hamiltonian numerical audit}]{numerical_audit.py}
\embedfile[desc={Trefoil Hamiltonian numerical audit output}]{numerical_audit.json}
\embedfile[desc={Trefoil curved radial-area audit}]{curved_area_audit.py}
\embedfile[desc={Trefoil curved radial-area audit output}]{curved_area_audit.json}
\embedfile[desc={Trefoil flat-mesh area audit}]{mesh_area_audit.py}
\embedfile[desc={Trefoil flat-mesh area audit output}]{mesh_area_audit.json}
