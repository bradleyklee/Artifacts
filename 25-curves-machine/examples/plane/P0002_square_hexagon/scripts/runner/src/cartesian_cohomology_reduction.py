...
It works with alpha=2H(p,q), rho=(2H)_p=2H_p, and the invariant
period form omega = dq/H_p = 2*dq/rho.
...
Current backend assumptions:
  ...
  * the primitive pole divisor is initially restricted to rho^(2r-1).
...
def regression_triangle_square_energy() -> SparsePoly:
    """Return 2H for the completed triangle+square regression example."""
...
def derivative_numerators(energy: SparsePoly, order: int) -> List[SparsePoly]:
    """N_j for partial_alpha^j(2/rho)=N_j/rho^(2j+1)."""
    Ep = der(energy, 1)
    Epp = der(Ep, 1)
...
def exact_image_map(energy: SparsePoly, order: int):
    """Return V -> numerator of d(V/rho^(2r-1)) at denominator rho^(2r+1)."""
...
def common_derivative_numerators(energy: SparsePoly, order: int) -> List[SparsePoly]:
    """Put omega,...,partial_alpha^order omega over denominator rho^(2r+1)."""
    _, reduce_curve = curve_reducer(energy)
    Ep = der(energy, 1)
...
def derive_relation(
    ...
    # The exact columns are independent in the successful sector.
