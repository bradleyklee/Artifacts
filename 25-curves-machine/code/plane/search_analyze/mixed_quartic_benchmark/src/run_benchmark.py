...
SRC=str(pathlib.Path(__file__).resolve().parents[2]/'q2_order6_engine/src')
RED=SRC
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--workers',type=int,default=4);ap.add_argument('--limit',type=int,default=0);ap.add_argument('--recompute',action='store_true');ns=ap.parse_args()
    idx=json.load(open(os.path.join(ROOT,'candidate_index.json'))); paths=[x['path'] for x in idx if ns.recompute or not os.path.exists(os.path.join(DATADIR,x['example_id']+'_result.json'))]
