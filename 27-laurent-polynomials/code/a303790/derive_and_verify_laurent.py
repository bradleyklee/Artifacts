...
OUT = Path(__file__).resolve().parents[2] / "examples" / "public" / "A303790" / "certificates"
...
# Verify direct constant terms for the first ten coefficients by finite
# one-variable Laurent convolution. Since Phi=B(w)C(y), the bivariate
# constant term is [B^k]_0 [C^k]_0. This avoids constructing the enormous
# fully expanded bivariate powers while checking exactly the same identity.
def laurent_dict(expression, variable):
    result = {}
    for term in sp.Add.make_args(sp.expand(expression)):
        ...
        coefficient = sp.cancel(term / variable**exponent)
...
B_dict = laurent_dict(B, w)
C_dict = laurent_dict(C, y)
B_power = {0: sp.S.One}
C_power = {0: sp.S.One}
direct = []
for k in range(10):
    direct.append(sp.Integer(B_power.get(0, 0) * C_power.get(0, 0)))
    B_power = laurent_multiply(B_power, B_dict)
    C_power = laurent_multiply(C_power, C_dict)
assert direct == A[:10]
...
(OUT / "laurent_certificate.json").write_text(
