...
// billiard model used by Artifact 18.  All production evolution happens here in
...
func (x F) Mul(y F) F {
	// Dodecagon burns stay in Q(sqrt(3)).  Preserve the same embedded
	// representation while avoiding the zero-coordinate products in the full
	// biquadratic formula.  This is an exact semantic fast path, not a change
	// of field or collision arithmetic.
	if x.C[1].Sign() == 0 && x.C[3].Sign() == 0 && y.C[1].Sign() == 0 && y.C[3].Sign() == 0 {
		a, c := x.C[0], x.C[2]
		e, g := y.C[0], y.C[2]
		return NewF(
			ratAdd(ratMul(a, e), ratScale(ratMul(c, g), 3)),
			ratInt(0),
			ratAdd(ratMul(a, g), ratMul(c, e)),
			ratInt(0),
		)
	}
...
func (x F) Sign() int {
	// Exact Q(sqrt(3)) fast path used heavily by dodecagon runs.
	if x.C[1].Sign() == 0 && x.C[3].Sign() == 0 {
		sa, sc := x.C[0].Sign(), x.C[2].Sign()
		if sc == 0 {
			return sa
		}
		if sa == 0 {
		...
		delta := ratSub(ratMul(x.C[0], x.C[0]), ratScale(ratMul(x.C[2], x.C[2]), 3))
		if sa > 0 {
			return delta.Sign()
		}
		return -delta.Sign()
...
func (x F) Inv() F {
	if x.C[1].Sign() == 0 && x.C[3].Sign() == 0 {
		den := ratSub(ratMul(x.C[0], x.C[0]), ratScale(ratMul(x.C[2], x.C[2]), 3))
		if den.Sign() == 0 {
			panic("inverse of zero field element")
		}
		return NewF(new(big.Rat).Quo(x.C[0], den), ratInt(0), new(big.Rat).Quo(ratNeg(x.C[2]), den), ratInt(0))
	}
	A := q2from(x.C[0], x.C[1])
	B := q2from(x.C[2], x.C[3])
