...
In calculus we divide by $\delta$ and send $\delta\to0$, so the correction
...
$x$ raises degree, and for bounded exponents the grade layers close through
bounded zero sums. But in the discrete square-gap case the step is fixed at
$\delta=1$, so the correction term remains: $(x+1)^2-x^2=2x+1$. This is
...
Rather than shifting the function value, it is more natural here to shift the
index value. For triangular numbers,
