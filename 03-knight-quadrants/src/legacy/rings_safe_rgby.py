...
def generate_four_color(points, trusted_last_index: int):
    owned = [set(), set(), set(), set()]
    occupied = set()
    cursor = [-1, -1, -1, -1]
...
def draw_safe_rgby(inner_half: int, out_path: Path):
    ...
    owned = generate_four_color(points, trusted_last_index)
...
def main():
    ...
    ap.add_argument("--out", type=Path, default=Path("rings_safe_rgby.png"))
    args = ap.parse_args()
    draw_safe_rgby(args.inner_half, args.out)
