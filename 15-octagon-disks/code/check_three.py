#!/usr/bin/env python3
"""
Independent exact checker for hard-octagons-L4-N3 event ledgers.
...
def check(certificate_path):
    ...
    if cert.get("schema") != "hard-octagons-L4-N3-self-contained-certificate/v1":
        raise ValueError("unsupported certificate schema")
    inst=cert["instance"]
    evo=cert["evolution"]
    initial=parse_state(inst["initial_state"])
    box=Q.parse(inst["container_half_box"])
    run={
        "ledger":evo["ledger"],
        "complexity_cutoff_bits":evo.get("complexity_cutoff_bits",0),
        "stop_class":evo.get("stop_class",""),
        "next_complexity_bits":evo.get("next_complexity_bits",0),
        "completed_batches":evo.get("completed_batches",0),
        "max_complexity_bits_observed":evo.get("max_complexity_bits_observed",0),
        "mask_bits":inst.get("class",-1),
    }
    ...
    return {
      "schema":"hard-octagons-L4-N3-independent-check/v1","certificate":str(certificate_path),"mask_bits":run["mask_bits"],
...
def main():
    ap=argparse.ArgumentParser(description="Independent exact checker for one self-contained L4 N3 certificate.")
    ap.add_argument("certificate",type=Path)
    ap.add_argument("--out",type=Path,required=True)
    a=ap.parse_args()
    report=check(a.certificate)
    a.out.parent.mkdir(parents=True,exist_ok=True)
    a.out.write_text(json.dumps(report,indent=2)+"\n")
    print(f"class={report['mask_bits']} rows={report['completed_batches']} ok={report['all_completed_rows_ok']} stop={report['terminal_class_independent']} retain={report['retained_for_ternary_comparison']}")
