...
{\footnotesize Typogeometric constructor codes and multiplicities: \(\Delta_{2}\!:\,2,\;\Delta_{3}\!:\,10,\;\Delta_{4}\!:\,25,\;\Delta_{5}\!:\,25\).}

\sectionbar{Coefficient and contour extraction}
{\footnotesize \[\begin{gathered}\mathcal K_n=\{(k_1,\ldots,k_4)\in\mathbb Z_{\ge0}^{4}:\sum_{j=1}^{4}jk_j=n-1\},\quad K=\sum_{j=1}^{4}k_j,\\a(n)=\frac{5}{n}\sum_{\boldsymbol k\in\mathcal K_n}\binom{n+K-1}{K}\binom{K}{k_1,\ldots,k_4}2^{k_1} 10^{k_2} 25^{k_3} 25^{k_4}.\end{gathered}\]}
\(a(n)=\dfrac{5}{2\pi i n}\oint_\gamma\dfrac{du}{\rho(u)^n},\quad n\ge1.\)

\sectionbar{Recurrence}
{\footnotesize \(\sum_{r=0}^{4}P_r(n)a(n+r)=0\) for \(n\ge 1\). The exact degree-4 coefficient polynomials are embedded in \texttt{certificate\_payload.json} (source: \texttt{data/recurrence.json}).}
