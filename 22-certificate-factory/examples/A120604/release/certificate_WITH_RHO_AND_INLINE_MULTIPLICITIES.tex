...
{\footnotesize \(\sum_{r=0}^{7}P_r(n)a(n+r)=0\) for \(n\ge 1\). The exact degree-7 coefficient polynomials are embedded in \texttt{certificate\_payload.json} (source: \texttt{release/certificate\_payload.json.gz\#/objects/p\_recurrence}).}

\sectionbar{Differential equation}
{\footnotesize The scalar linear ODE has order 7. It is obtained from the recurrence by the standard Euler-operator substitution. Exact coefficients and boundary polynomial: \texttt{release/certificate\_payload.json.gz\#/objects/ode\_from\_recurrence}.}
