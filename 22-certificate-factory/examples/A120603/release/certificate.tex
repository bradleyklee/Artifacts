...
{\footnotesize \(\sum_{r=0}^{6}P_r(n)a(n+r)=0\) for \(n\ge 1\). The exact degree-6 coefficient polynomials are embedded in \texttt{certificate\_payload.json} (source: \texttt{data/recurrence.json}).}

\sectionbar{Differential equation}
{\footnotesize The scalar linear ODE has order 6. It is obtained from the recurrence by the standard Euler-operator substitution. Exact coefficients and boundary polynomial: \texttt{data/ode.json}.}
